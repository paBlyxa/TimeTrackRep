...
<div class="timesheetForm">
  <h1>Новая запись</h1>
  <form:form method="POST" modelAttribute="timesheetForm" onSubmit="return validate()">
	<table class="newRecordTable">
		...
		<tbody>
			<tr>
				<td>
					<form:input type="text" required="true" class="datepicker-here input" id="datepickerTS"
				...
				<td>
					<form:select path="projectId" class="input" required="true" id="selectProject">
						<form:option value="" label="--Выберите проект"/>
              			<form:options items="${projectList}" itemValue="projectId" itemLabel="name"/>
              		</form:select>
				</td>
				<td>
					<form:select path="taskId" class="input" required="true" id="selectTask">
						<form:option value="" label="--Выберите задачу"/>
              			<form:options items="${taskList}" itemValue="taskId" itemLabel="name"/>
              		</form:select>
				</td>
				<td><form:input class="input" type="text" path="countTime" list="listTimes" required="true" id="inputCountTime"/>
				...
				<c:forEach var="timesheetByDay" items="${timesheetsByDays}" varStatus="stat">
					...
					<tr class="${classRow}">
						<td rowspan="${fn:length(timesheetByDay.timesheets)}">
							<fmt:parseDate value="${timesheetByDay.date}" pattern="yyyy-MM-dd" var="parsedDate" type="date"/>
