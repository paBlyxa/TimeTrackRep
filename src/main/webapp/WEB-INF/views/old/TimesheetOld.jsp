	...
	<ul class="timesheetList">
		<c:forEach items="${timesheetList}" var="timesheet">
			<li id="timesheet_<c:out value="timesheet.id"/>">
				<div class="timesheetProject">
					<c:out value="${timesheet.project.name}" />
				</div>
				<div>
					<span class="timesheetDate"><c:out
							value="${timesheet.dateTask}" /></span> <span
						class="timesheetCountTime"><c:out
							value="${timesheet.countTime}" /></span>
				</div>
			</li>
		</c:forEach>
	</ul>
